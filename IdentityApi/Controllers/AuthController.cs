...
using UAParser;
using UserAgent = IdentityApi.RequestModels.UserAgent;
...
public class AuthController : Controller
{
    ...
    public async Task<IActionResult> LogInAsync([FromBody] LogInRequest logInRequest)
    {
        ...
        {
            var token = _authManager.GenerateJwtToken(logInResponse);
            ...
            if (logInResponse.IS_VERIFIED)
            {
                ...
                logInResponse.TOKEN.REFRESH = _authManager
                    .GetOrCreateRefreshToken(logInResponse.PROFILE_ID, GetClientInfo(), GetIPAddress()).TOKEN;
            }
    ...
    public async Task<IActionResult> RefreshTokenAsync(RefreshTokenRequest refreshTokenRequest)
    {
        ...
        var storedRefreshToken = _authManager.GetRefreshToken(profileID, GetClientInfo(), GetIPAddress());

        if (storedRefreshToken != null &&
            !storedRefreshToken.TOKEN.Equals(refreshTokenRequest.REFRESH, StringComparison.OrdinalIgnoreCase))
        {
        ...
        var tokenResponse = await _policyApiClient.GetPolicyTokenAsync(header);
    ...
    public IActionResult LogOut()
    {
        ...
        var success = _authManager.Logout(profileID, GetClientInfo(), GetIPAddress());
    ...
    private UserAgent GetClientInfo()
    {
        UserAgent ua = new();

        var userAgent = HttpContext.Request.Headers["User-Agent"];
        var uaString = Convert.ToString(userAgent.FirstOrDefault());
    ...
    {
        return (HttpContext.User.Identity as ClaimsIdentity).FindFirst(ClaimTypes.NameIdentifier).Value;
    }
}
