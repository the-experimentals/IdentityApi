...
public class AccountController : Controller
{
    ...
    public AccountController(IAccountManager accountManager, IPwnedPasswordsClient pwnedPasswords,
        NotificationClient notificationClient, ICustomRazorEngine customRazorEngine)
    {
    ...
    public IActionResult CreateNewProfile([FromBody] NewProfileRequest newProfileView)
    {
        ...
        Profile userProfile = new()
        {
            ...
            PERSON = new Person
            {
                FIRST_NAME = newProfileView.FIRST_NAME.Trim(), LAST_NAME = newProfileView.LAST_NAME.Trim()
            }
        };

        userProfile.CREDENTIAL = new Credential
        {
            USERNAME = newProfileView.USERNAME.Trim().ToLower(), PASSWORD = newProfileView.PASSWORD
        };

        var result = _accountManager.CreateProfile(userProfile);
    ...
    public async Task<IActionResult> CheckPwnedPasswordAsync([FromQuery] CheckPwnedPasswordRequest passwordRequest)
    {
        ...
        var result = await _pwnedPasswords.HasPasswordBeenPwned(passwordRequest.PASSWORD.Trim());
        return Ok(result);
    }
    ...
    {
        var profileCardResponse = _accountManager.GetProfiles();
    ...
    public IActionResult ChangePassword([FromBody] ChangePasswordRequest changePasswordRequest)
    {
        ...
        var response = _accountManager.ChangePassword(profileID, changePasswordRequest);
    ...
    public async Task<IActionResult> SendVerificationCodeAsync()
    {
        ...
        var header = new Metadata { { "Authorization", $"{token}" } };
        ...
        var emailBody = await _customRazorEngine.RazorViewToHtmlAsync("EmailTemplate/OtpEmail.cshtml",
            new OtpView { OTP = _accountManager.GenerateOTP(profileID) });

        List<string> sendTO = new();
        sendTO.Add(profile.EMAIL);
        var result = await _notificationClient.SendEmailAsync(
            new EmailRequest { TO = sendTO, SUBJECT = "Test", CONTENT = emailBody, HTML = true }, header);
    ...
    {
        var isDeleted = _accountManager.DeleteProfile(deleteProfile.PROFILE_ID);
